...
class pnt(hoomd.hpmc.integrate.mode_hpmc):
    R""" HPMC integration for spheres (2D/3D), supporting swap moves.
    ...
    * *ignore_overlaps* (**default: False**) - set to True to disable overlap checks between this and other types with *ignore_overlaps=True*
    ...
    def __init__(self, seed, d=0.1, a=0.1, move_ratio=0.5, nselect=4, swap_mode="diameter", soft_mode="hard",dr_reject=np.inf,implicit=False, depletant_mode='circumsphere',restore_state=False):
        ...
        #    else:
        self.cpp_integrator = _hpmc_swap_plugin.IntegratorHPMCMonoPointSwap(hoomd.context.current.system_definition, seed);
        #else:
    ...
    def test_overlap(self,type_i, type_j, rij, qi, qj, tag_i, tag_j, use_images=True, exclude_self=False):
        ...
        return self.cpp_integrator.py_test_overlap_plugin(ti,tj,rij,qi,qj,tag_i, tag_j, use_images,exclude_self)
