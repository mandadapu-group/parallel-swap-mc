...
class ludovic(object):
    ...
    '''
    def __init__(self, mc, kT, scaledr_cut=1.25, v0=1.0, eps=0.2, model='nature2019', llvm_ir_file=None, clang_exec=None):
        hoomd.util.print_status_line();
        ...
        if (model == "nature2019"):
            ...
            code = """
                          ...
                          """.format(eps,scaledr_cut,v0/kT,c[0]/kT,c[1]/kT,c[2]/kT);
        elif (model == "nature_n=18"):
            a = scaledr_cut
            A = np.array([  [1,a**2,a**4],
                            [0,2*a,4*a**3],
                            [0,2,12*a**2]
                            ])
            b = np.array([-v0*(1/a**18),v0*(18*v0/a**17),-v0*(306/a**18)])
            c = np.linalg.solve(A,b)
            code = """
                            float rsq = dot(r_ij, r_ij);
                            float sigma  = 0.5*( d_i+d_j)*(1-{}*fabs(d_i-d_j) );
