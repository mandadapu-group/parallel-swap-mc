...
class IntegratorHPMCPolydisperse : public IntegratorHPMCSwap
    ...
    public:
        ...
        */
         ...
         */
        /*
        virtual bool py_test_overlap_plugin(unsigned int type_i, unsigned int type_j,
        ...
        */
        ...
        virtual void prepRun(unsigned int timestep)
            {
            // base class method
            IntegratorHPMCSwap::prepRun(timestep);
...
IntegratorHPMCPolydisperse<Shape>::IntegratorHPMCPolydisperse(std::shared_ptr<SystemDefinition> sysdef,
                                                   unsigned int seed)
            : IntegratorHPMCSwap(sysdef, seed),
...
std::vector< std::string > IntegratorHPMCPolydisperse<Shape>::getProvidedLogQuantities()
    ...
    std::vector< std::string > result = IntegratorHPMCSwap::getProvidedLogQuantities();
    // then add ours
    if(m_patch)
        {
...
Scalar IntegratorHPMCPolydisperse<Shape>::getLogValue(const std::string& quantity, unsigned int timestep)
    ...
    if (quantity == "hpmc_patch_energy")
        ...
        return IntegratorHPMCSwap::getLogValue(quantity, timestep);
        }
    }
...
void IntegratorHPMCPolydisperse<Shape>::slotNumTypesChange()
    ...
    IntegratorHPMCSwap::slotNumTypesChange();
...
void IntegratorHPMCPolydisperse<Shape>::update(unsigned int timestep)
    ...
    if (m_soft_mode)
    {
        m_exec_conf->msg->notice(5) << "You're in soft mode, so they can overlap! " << std::endl;
    }
    ...
    // get needed vars
    ArrayHandle<hpmc_counters_swap_t> h_counters(m_count_total, access_location::host, access_mode::readwrite);
    ...
    */
    ...
    for (unsigned int i_nselect = 0; i_nselect < m_nselect; i_nselect++)
        ...
        for (unsigned int cur_particle = 0; cur_particle < m_pdata->getN(); cur_particle++)
            ...
            OverlapReal diameter_old = h_diameter.data[i];
            ...
            bool overlap=false;
            OverlapReal r_cut_patch_ij = 0;
            OverlapReal r_cut_patch_i_swapj = 0;

            if (m_patch && !m_patch_log)
                {
                r_cut_patch_ij = m_patch->getScaledRCut()*0.5*diameter_i;
                                ...
                                if (m_aabb_tree.isNodeLeaf(cur_node_idx))
                                    ...
                                    for (unsigned int cur_p = 0; cur_p < m_aabb_tree.getNodeNumParticles(cur_node_idx); cur_p++)
                                        ...
                                        if (!m_soft_mode && (h_overlaps.data[m_overlap_idx(typ_i, typ_j)]
                                            ...
                                            && (check_circumsphere_overlap_plugin(r_ij, diameter_i, diameter_j)
                                            || check_circumsphere_overlap_plugin(r_ij_swap, diameter_swap, diameter_j_swap))
                                            && (test_overlap_plugin(r_ij, diameter_i, diameter_j, counters.overlap_err_count)
                                            || test_overlap_plugin(r_ij_swap, diameter_swap, diameter_j_swap, counters.overlap_err_count)))
                                            {
                                                overlap = true;
                                                break;
                                            }
                                ...
                                if (detail::overlap(m_aabb_tree.getNodeAABB(cur_node_idx), aabb))
                                    ...
                                    if (m_aabb_tree.isNodeLeaf(cur_node_idx))
                                        ...
                                        for (unsigned int cur_p = 0; cur_p < m_aabb_tree.getNodeNumParticles(cur_node_idx); cur_p++)
                                            ...
                                            unsigned int typ_j = __scalar_as_int(postype_j.w);
                                            //Shape shape_j(quat<Scalar>(orientation_j), m_params[typ_j]);
                                            OverlapReal diameter_j = h_diameter.data[j];//shape_j.getCircumsphereDiameter(tag_j);
                            ...
                            if (detail::overlap(m_aabb_tree.getNodeAABB(cur_node_idx), aabb) || detail::overlap(m_aabb_tree.getNodeAABB(cur_node_idx), aabbswap))
                                ...
                                if (m_aabb_tree.isNodeLeaf(cur_node_idx))
                                    ...
                                    for (unsigned int cur_p = 0; cur_p < m_aabb_tree.getNodeNumParticles(cur_node_idx); cur_p++)
                                        ...
                                        if ( j != i  && j != i_swap) 
                                        ...
                                        else if ( j != i  && j == i_swap)
                                            {
                                                ...
                                                if (cur_image == 0)
                                                    ...
                                                    {
                                                    // If this is particle i and we are in an outside image, use the translated position and orientation
                                                    postype_j_swap = make_scalar4(pos_swap_old.x, pos_swap_old.y, pos_swap_old.z, postype_swap_old.w);
                                        ...
                                        unsigned int typ_j_swap = __scalar_as_int(postype_j_swap.w);
                                        Shape shape_j_swap(quat<Scalar>(orientation_j_swap), m_params[typ_j_swap]);
                                        OverlapReal diameter_j_swap = h_diameter.data[j];//shape_j_swap.getCircumsphereDiameter(tag_j);
                                        //unsigned int typ_j = __scalar_as_int(postype_j.w);
                                        //Shape shape_j(quat<Scalar>(orientation_j), m_params[typ_j]);
                                        Scalar rcut = r_cut_patch_ij + 0.5 * m_patch->getScaledRCut()*diameter_j;
                                        ...
                                        if (m_soft_mode && (dot(r_ij,r_ij) <= rcut*rcut || dot(r_ij_swap,r_ij_swap) <= rcut_swap*rcut_swap) ) 
                                            {
                                                ...
                                                { 
                                                    ...
                                                    patch_field_energy_diff += m_patch->energy(r_ij, typ_i,
                                                                               ...
                                                                               //h_diameter.data[i],
                                                                               diameter_old,//shape_old.getCircumsphereDiameter(tag_i_old),
                                                                               h_charge.data[i],
                                                                               typ_j,
                                                                               quat<float>(orientation_j),
                                                                               //h_diameter.data[j],
                                                                               diameter_j,//shape_j.getCircumsphereDiameter(tag_j),
                                                                               h_charge.data[j]
                                                                               );
                                                }
                                                else if ( dot(r_ij_swap,r_ij_swap) <= rcut_swap*rcut_swap &&  dot(r_ij,r_ij) > rcut*rcut)
                                                {
                                                    patch_field_energy_diff += m_patch->energy(r_ij_swap, typ_swap,
                                                                               ...
                                                                               //h_diameter.data[i],
                                                                               diameter_swap_old,//shape_swap_old.getCircumsphereDiameter(tag_swap_old),
                                                                               h_charge.data[i_swap],
                                                                               typ_j,
                                                                               quat<float>(orientation_j),
                                                                               //h_diameter.data[j],
                                                                               diameter_j_swap,//shape_j_swap.getCircumsphereDiameter(tag_j),
                                                                               h_charge.data[j]
                                                                               );
                                                }
                                                else if ( dot(r_ij_swap,r_ij_swap) <= rcut_swap*rcut_swap &&  dot(r_ij,r_ij) <= rcut*rcut)
...
Scalar IntegratorHPMCPolydisperse<Shape>::getMaxCoreDiameter()
    ...
    for (unsigned int typ = 0; typ < this->m_pdata->getNTypes(); typ++)
        ...
        maxD = std::max(maxD, temp.getMaxCircumsphereDiameter());
        }
...
OverlapReal IntegratorHPMCPolydisperse<Shape>::getMinCoreDiameter()
    ...
    for (unsigned int typ = 0; typ < this->m_pdata->getNTypes(); typ++)
        {
        Shape temp(quat<Scalar>(), m_params[typ]);
        minD = std::min(minD, temp.getMinCircumsphereDiameter());
        }
    ...
    Scalar max_trans_d_and_diam(0.0);
       ...
       // for each type, create a temporary shape and return the maximum sum of diameter and move size
        for (unsigned int typ = 0; typ < this->m_pdata->getNTypes(); typ++)
            ...
            Scalar range_i = detail::max((Scalar)temp.getMaxCircumsphereDiameter(),r_cut_patch);
...
bool IntegratorHPMCPolydisperse<Shape>::py_test_overlap_plugin(unsigned int type_i, unsigned int type_j,
    ...
    if (use_images)
        ...
        if (m_pdata->getDomainDecomposition())
            {
            this->m_exec_conf->msg->error() << "test_overlap_plugin does not support MPI parallel jobs with use_images=True" << std::endl;
            throw std::runtime_error("test_overlap_plugin does not support MPI parallel jobs");
            }
        #endif
        ...
        for (unsigned int cur_image = 0; cur_image < n_images; cur_image++)
            ...
            if (check_circumsphere_overlap_plugin(dr + m_image_list[cur_image], shape_i, shape_j) &&
