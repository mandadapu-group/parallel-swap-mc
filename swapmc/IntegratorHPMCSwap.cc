...
IntegratorHPMCSwap::IntegratorHPMCSwap(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Integrator(sysdef, 0.005), m_seed(seed),  m_move_prob(1.0), m_nselect(4),
      ...
      #endif
    {
    m_exec_conf->msg->notice(5) << "Constructing IntegratorHPMCSwap" << endl;
    ...
    GPUArray<hpmc_counters_swap_t> counters(1, this->m_exec_conf);
    ...
    m_pdata->getNumTypesChangeSignal().connect<IntegratorHPMCSwap, &IntegratorHPMCSwap::slotNumTypesChange>(this);

    resetStats();
    }

IntegratorHPMCSwap::~IntegratorHPMCSwap()
    {
    m_exec_conf->msg->notice(5) << "Destroying IntegratorHPMCSwap" << endl;
    m_pdata->getNumTypesChangeSignal().disconnect<IntegratorHPMCSwap, &IntegratorHPMCSwap::slotNumTypesChange>(this);

    #ifdef ENABLE_MPI
    if (m_communicator_ghost_width_connected)
        m_comm->getGhostLayerWidthRequestSignal().disconnect<IntegratorHPMCSwap, &IntegratorHPMCSwap::getGhostLayerWidth>(this);
    if (m_communicator_flags_connected)
        m_comm->getCommFlagsRequestSignal().disconnect<IntegratorHPMCSwap, &IntegratorHPMCSwap::getCommFlags>(this);
    #endif
    }


void IntegratorHPMCSwap::slotNumTypesChange()
    {
    // old size of arrays
...
*/
std::vector< std::string > IntegratorHPMCSwap::getProvidedLogQuantities()
    {
    ...
    result.push_back("hpmc_swap_acceptance");
...
*/
Scalar IntegratorHPMCSwap::getLogValue(const std::string& quantity, unsigned int timestep)
    {
    hpmc_counters_swap_t counters = getCounters(2);

    if (quantity == "hpmc_sweep")
        {
        hpmc_counters_swap_t counters_total = getCounters(0);
    ...
    else if (quantity == "hpmc_swap_acceptance")
        {
...
*/
bool IntegratorHPMCSwap::checkParticleOrientations()
    {
    m_exec_conf->msg->notice(5) << "Spherical polydisperse particles don't need to check orientations normalization" << endl;
    return true;//result;
    }
...
*/
bool IntegratorHPMCSwap::attemptBoxResize(unsigned int timestep, const BoxDim& new_box)
    {
...
/*! \param mode 0 -> Absolute count, 1 -> relative to the start of the run, 2 -> relative to the last executed step
    ...
    IntegratorHPMCSwap maintains a count of the number of accepted and rejected moves since instantiation. getCounters()
...
*/
hpmc_counters_swap_t IntegratorHPMCSwap::getCounters(unsigned int mode)
    {
    ArrayHandle<hpmc_counters_swap_t> h_counters(m_count_total, access_location::host, access_mode::read);
    ...
    if (m_comm)
        ...
        MPI_Allreduce(MPI_IN_PLACE, &result.swap_accept_count, 1, MPI_LONG_LONG_INT, MPI_SUM, m_exec_conf->getMPICommunicator());
        MPI_Allreduce(MPI_IN_PLACE, &result.swap_reject_count, 1, MPI_LONG_LONG_INT, MPI_SUM, m_exec_conf->getMPICommunicator());
...
void export_IntegratorHPMCSwap(py::module& m)
    {
   py::class_<IntegratorHPMCSwap, std::shared_ptr< IntegratorHPMCSwap > >(m, "IntegratorHPMCSwap", py::base<Integrator>(), py::module_local())
    ...
    .def("checkParticleOrientations", &IntegratorHPMCSwap::checkParticleOrientations)
    .def("getMPS", &IntegratorHPMCSwap::getMPS)
    .def("getCounters", &IntegratorHPMCSwap::getCounters)
    .def("communicate", &IntegratorHPMCSwap::communicate)
    .def("slotNumTypesChange", &IntegratorHPMCSwap::slotNumTypesChange)
    .def("setDeterministic", &IntegratorHPMCSwap::setDeterministic)
    .def("disablePatchEnergyLogOnly", &IntegratorHPMCSwap::disablePatchEnergyLogOnly)
    ;

   py::class_< hpmc_counters_swap_t >(m, "hpmc_counters_swap_t")
    .def_readwrite("translate_accept_count", &hpmc_counters_swap_t::translate_accept_count)
    .def_readwrite("translate_reject_count", &hpmc_counters_swap_t::translate_reject_count)
    .def_readwrite("rotate_accept_count", &hpmc_counters_swap_t::rotate_accept_count)
    .def_readwrite("rotate_reject_count", &hpmc_counters_swap_t::rotate_reject_count)
    .def_readwrite("swap_accept_count", &hpmc_counters_swap_t::swap_accept_count)
    .def_readwrite("swap_reject_count", &hpmc_counters_swap_t::swap_reject_count)
    .def_readwrite("overlap_checks", &hpmc_counters_swap_t::overlap_checks)
    .def("getTranslateAcceptance", &hpmc_counters_swap_t::getTranslateAcceptance)
    .def("getRotateAcceptance", &hpmc_counters_swap_t::getRotateAcceptance)
    .def("getSwapAcceptance", &hpmc_counters_swap_t::getSwapAcceptance)
    .def("getNMoves", &hpmc_counters_swap_t::getNMoves)
    ;
    }
