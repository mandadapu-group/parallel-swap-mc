...
// inclusion guard
#ifndef _INTEGRATOR_HPMC_SWAP_H_
#define _INTEGRATOR_HPMC_SWAP_H_
...
class PYBIND11_EXPORT IntegratorHPMCSwap : public Integrator
    {
    public:
        //! Constructor
        IntegratorHPMCSwap(std::shared_ptr<SystemDefinition> sysdef,
                       unsigned int seed);

        virtual ~IntegratorHPMCSwap();
        ...
        */
        ...
        void setSwapMode(std::string swap_mode)
            {
                if (swap_mode == "diameter") //swap diameters not position
                {
                    m_swap_mode = true;
                }
                else if (swap_mode == "position")
                {
                    m_swap_mode = false;
                }
            }
        
        void setSoftMode(std::string soft_mode)
            {
                if (soft_mode == "soft") //particles have zero hard shape radius
                {
                    m_soft_mode = true;
                }
                else if (soft_mode == "hard")
                {
                    m_soft_mode = false;
                }
            }
        
        void setdrReject(Scalar dr_reject)
            {
                m_dr_reject = dr_reject;
            }
        ...
        */
        ...
        virtual void printStats()
            {
            hpmc_counters_swap_t counters = getCounters(1);
            ...
            if (counters.rotate_accept_count + counters.rotate_reject_count != 0)
                {
                m_exec_conf->msg->notice(2) << "Average rotate acceptance:    " << counters.getSwapAcceptance() << std::endl;
                }
        ...
        */
        ...
        hpmc_counters_swap_t getCounters(unsigned int mode=0);
    ...
    protected:
        ...
        GPUArray< hpmc_counters_swap_t > m_count_total;  //!< Accept/reject total count
        ...
        bool m_swap_mode; //<- If true then we are swapping diameters instead of position
        bool m_soft_mode; //<- If true then we are swapping diameters instead of position
        Scalar m_dr_reject; //<- additional rejection criterion, if the difference in diameters of two swapped particles are too large
        ...
        */
        virtual void setCommunicator(std::shared_ptr<Communicator> comm)
            ...
            if (! m_communicator_ghost_width_connected)
                ...
                assert(comm);
                comm->getGhostLayerWidthRequestSignal().connect<IntegratorHPMCSwap, &IntegratorHPMCSwap::getGhostLayerWidth>(this);
            ...
            if (! m_communicator_flags_connected)
                ...
                // only add the migrate request on the first call
                assert(comm);
                comm->getCommFlagsRequestSignal().connect<IntegratorHPMCSwap, &IntegratorHPMCSwap::getCommFlags>(this);
    ...
    private:
        hpmc_counters_swap_t m_count_run_start;             //!< Count saved at run() start
        hpmc_counters_swap_t m_count_step_start;            //!< Count saved at the start of the last step
...
void export_IntegratorHPMCSwap(pybind11::module& m);

} // end namespace hpmc

#endif // _INTEGRATOR_HPMC_SWAP_H_
