...
/*! \file UpdaterBoxMCSwap.cc
    \brief Definition of UpdaterBoxMCSwap
*/

namespace hpmc
{

UpdaterBoxMCSwap::UpdaterBoxMCSwap(std::shared_ptr<SystemDefinition> sysdef,
                             std::shared_ptr<IntegratorHPMCSwap> mc,
        ...
        : Updater(sysdef),
          ...
          m_seed(seed)
    {
    m_exec_conf->msg->notice(5) << "Constructing UpdaterBoxMCSwap" << std::endl;
    ...
    m_pdata->getMaxParticleNumberChangeSignal().connect<UpdaterBoxMCSwap, &UpdaterBoxMCSwap::slotMaxNChange>(this);

    }

UpdaterBoxMCSwap::~UpdaterBoxMCSwap()
    {
    m_exec_conf->msg->notice(5) << "Destroying UpdaterBoxMCSwap" << std::endl;
    m_pdata->getMaxParticleNumberChangeSignal().disconnect<UpdaterBoxMCSwap, &UpdaterBoxMCSwap::slotMaxNChange>(this);
    }

/*! hpmc::UpdaterBoxMCSwap provides:
...
*/
std::vector< std::string > UpdaterBoxMCSwap::getProvidedLogQuantities()
    {
...
*/
Scalar UpdaterBoxMCSwap::getLogValue(const std::string& quantity, unsigned int timestep)
    {
    hpmc_boxmc_counters_swap_t counters = getCounters(1);
...
*/
inline bool UpdaterBoxMCSwap::is_oversheared()
    {
...
*/
inline bool UpdaterBoxMCSwap::remove_overshear()
    {
...
*/
inline bool UpdaterBoxMCSwap::box_resize_trial(Scalar Lx,
                                          Scalar Ly,
                                          Scalar Lz,
                                          Scalar xy,
                                          Scalar xz,
...
inline bool UpdaterBoxMCSwap::safe_box(const Scalar newL[3], const unsigned int& Ndim)
    {
...
/*! Perform Metropolis Monte Carlo box resizes and shearing
    \param timestep Current time step of the simulation
*/
void UpdaterBoxMCSwap::update(unsigned int timestep)
    {
    if (m_prof) m_prof->push("UpdaterBoxMCSwap");
    m_count_step_start = m_count_total;
    m_exec_conf->msg->notice(10) << "UpdaterBoxMCSwap: " << timestep << std::endl;

    // Create a prng instance for this timestep
    // While the name of everything is chaned with Swap on it, I still use the same RNGIdentifer
    ...
    if (range == 0.0)
        ...
        m_exec_conf->msg->warning() << "No move types with non-zero weight. UpdaterBoxMCSwap has nothing to do." << std::endl;
    ...
    if (move_type_select < m_Volume_weight)
        ...
        m_exec_conf->msg->warning() << "UpdaterBoxMCSwap selected an unassigned move type. Selected " << move_type_select << " from range " << range << std::endl;
        if (m_prof) m_prof->pop();
        return;
        }

    if (m_prof) m_prof->push("UpdaterBoxMCSwap: examining shear");
    if (is_oversheared())
        {
...
void UpdaterBoxMCSwap::update_L(unsigned int timestep, hoomd::RandomGenerator& rng)
    {
    if (m_prof) m_prof->push("UpdaterBoxMCSwap: update_L");
...
void UpdaterBoxMCSwap::update_lnV(unsigned int timestep, hoomd::RandomGenerator& rng)
    {
    if (m_prof) m_prof->push("UpdaterBoxMCSwap: update_lnV");
    // Get updater parameters for current timestep
    Scalar P = m_P->getValue(timestep);
...
void UpdaterBoxMCSwap::update_V(unsigned int timestep, hoomd::RandomGenerator& rng)
    {
    if (m_prof) m_prof->push("UpdaterBoxMCSwap: update_V");
    // Get updater parameters for current timestep
    Scalar P = m_P->getValue(timestep);
...
void UpdaterBoxMCSwap::update_shear(unsigned int timestep, hoomd::RandomGenerator& rng)
    {
    if (m_prof) m_prof->push("UpdaterBoxMCSwap: update_shear");
    // Get updater parameters for current timestep
    // Get current particle data and box lattice parameters
    assert(m_pdata);
    unsigned int Ndim = m_sysdef->getNDimensions();
...
void UpdaterBoxMCSwap::update_aspect(unsigned int timestep, hoomd::RandomGenerator& rng)
    {
    ...
    if (m_prof) m_prof->push("UpdaterBoxMCSwap: update_aspect");
    // Get updater parameters for current timestep
    // Get current particle data and box lattice parameters
    assert(m_pdata);
    unsigned int Ndim = m_sysdef->getNDimensions();
...
/*! \param mode 0 -> Absolute count, 1 -> relative to the start of the run, 2 -> relative to the last executed step
    ...
    UpdaterBoxMCSwap maintains a count of the number of accepted and rejected moves since instantiation. getCounters()
...
*/
hpmc_boxmc_counters_swap_t UpdaterBoxMCSwap::getCounters(unsigned int mode)
    {
...
void export_UpdaterBoxMCSwap(py::module& m)
    {
   py::class_< UpdaterBoxMCSwap, std::shared_ptr< UpdaterBoxMCSwap > >(m, "UpdaterBoxMCSwap", py::base<Updater>())
    .def(py::init< std::shared_ptr<SystemDefinition>,
                         std::shared_ptr<IntegratorHPMCSwap>,
                         std::shared_ptr<Variant>,
                         Scalar,
                         const unsigned int >())
    .def("volume", &UpdaterBoxMCSwap::volume)
    .def("ln_volume", &UpdaterBoxMCSwap::ln_volume)
    .def("length", &UpdaterBoxMCSwap::length)
    .def("shear", &UpdaterBoxMCSwap::shear)
    .def("aspect", &UpdaterBoxMCSwap::aspect)
    .def("printStats", &UpdaterBoxMCSwap::printStats)
    .def("resetStats", &UpdaterBoxMCSwap::resetStats)
    .def("getP", &UpdaterBoxMCSwap::getP)
    .def("setP", &UpdaterBoxMCSwap::setP)
    .def("get_volume_delta", &UpdaterBoxMCSwap::get_volume_delta)
    .def("get_ln_volume_delta", &UpdaterBoxMCSwap::get_ln_volume_delta)
    .def("get_length_delta", &UpdaterBoxMCSwap::get_length_delta)
    .def("get_shear_delta", &UpdaterBoxMCSwap::get_shear_delta)
    .def("get_aspect_delta", &UpdaterBoxMCSwap::get_aspect_delta)
//    .def("getMoveRatio", &UpdaterBoxMCSwap::getMoveRatio)
//    .def("getReduce", &UpdaterBoxMCSwap::getReduce)
//    .def("getIsotropic", &UpdaterBoxMCSwap::getIsotropic)
    .def("computeAspectRatios", &UpdaterBoxMCSwap::computeAspectRatios)
    .def("getCounters", &UpdaterBoxMCSwap::getCounters)
    ;

   py::class_< hpmc_boxmc_counters_swap_t >(m, "hpmc_boxmc_counters_swap_t")
    .def_readwrite("volume_accept_count", &hpmc_boxmc_counters_swap_t::volume_accept_count)
    .def_readwrite("volume_reject_count", &hpmc_boxmc_counters_swap_t::volume_reject_count)
    .def_readwrite("ln_volume_accept_count", &hpmc_boxmc_counters_swap_t::ln_volume_accept_count)
    .def_readwrite("ln_volume_reject_count", &hpmc_boxmc_counters_swap_t::ln_volume_reject_count)
    .def_readwrite("shear_accept_count", &hpmc_boxmc_counters_swap_t::shear_accept_count)
    .def_readwrite("shear_reject_count", &hpmc_boxmc_counters_swap_t::shear_reject_count)
    .def_readwrite("aspect_accept_count", &hpmc_boxmc_counters_swap_t::aspect_accept_count)
    .def_readwrite("aspect_reject_count", &hpmc_boxmc_counters_swap_t::aspect_reject_count)
    .def("getVolumeAcceptance", &hpmc_boxmc_counters_swap_t::getVolumeAcceptance)
    .def("getLogVolumeAcceptance", &hpmc_boxmc_counters_swap_t::getLogVolumeAcceptance)
    .def("getShearAcceptance", &hpmc_boxmc_counters_swap_t::getShearAcceptance)
    .def("getAspectAcceptance", &hpmc_boxmc_counters_swap_t::getAspectAcceptance)
    .def("getNMoves", &hpmc_boxmc_counters_swap_t::getNMoves)
    ;
    }
